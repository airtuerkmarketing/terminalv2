...
Highest decision: **D-111** (role model since rewritten — see below: super_admin=5 / department_admin=4 / ai_admin=2 / user=1, `admin` tier removed). RAG genuine quality **86.9%** (D-104, harness-measured).
...
> ⚠️ **Superseded by D-111 (2026-07-01).** Current: **89 migrations** applied + registered
> (file↔registry parity re-verified by hash); highest applied migration `20260701000000`
> (D-111 role rewrite); **highest decision D-111**; role model now `super_admin`=5 /
> `department_admin`=4 / `ai_admin`=2 / `user`=1 (`admin` tier drained + removed from the
> CHECK constraints); `audit_events` table added. The 2026-06-29 figures below are the
> point-in-time record for that date.
...
- Highest decision (at snapshot date): **D-110** (AI-Attach PDF/DOCX — code/edge-only); now **D-111** (see superseded note above)
...
For sprint-scoped detail of D-081–D-104 see §16 above; for D-105–D-111 detail see `DECISIONS.md`.
